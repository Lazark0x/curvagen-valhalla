...
A caveat that was raised and then measured: the ratchet's baseline might read as distinct partly because of *dirty* loops (the census served 25.6 % Fallback Loops). Excluding every loop with `edge_reuse_geom > 0.3` moves prod's `bank_overlap_mean` by 0.006 on the whole bank and by 0.000 on the served surface — negligible; the caveat does not hold. **On the served surface (blocks A+B, slots 0–5, both engines with the prod penalty) the gap is wider than on the bank:** prod 0.392 / 14.4 % near-dup, P1.1 knee 0.448 / 22.0 % (1.14×), **P2 v2 0.588 / 50.4 % (1.50×)** — half of P2's served loops have a > 60 % twin in the bank. Gate v2 (2026-09-07) writes ratchet 9 on that surface.
