...
_(d and bracket B follow as each lands. **d was re-based on c** — the 0.35 threshold is the likelier recommendation, so the budget axis is priced on it: `v8003-p21-d.json` = c at `eval_total` 1 200 / `relax_eval_cap` 150 + `rescue_last`; the `engine` note in `p2-1-d/report.json` was written before the re-base and still says "a2".)_
...
| d | c at 1 200 total + rescue-built last | _(§3)_ | _(§3)_ | _(§3)_ | _(§3)_ |
